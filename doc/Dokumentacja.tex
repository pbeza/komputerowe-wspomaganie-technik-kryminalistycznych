...
Projekt jest napisany w \emph{Javie} w wersji \emph{8}\footnote{Wykorzystano m.in. bibliotekę \texttt{java.time} i wyrażenia \emph{lambda}, które nie są dostępne w poprzednich wersjach \emph{Javy}. Domyślnie na \emph{Debianie} jest jeszcze stara wersja \emph{Java~7}, dlatego \emph{Javę~8} należało \href{http://www.webupd8.org/2014/03/how-to-install-oracle-java-8-in-debian.html}{doinstalować} samodzielnie.}, więc z natury powinien być przenośny pomiędzy wszystkimi systemami operacyjnymi, na których można zainstalować maszynę wirtualną \emph{Javy}. W rzeczywistości nie jest to takie proste, ponieważ w projekcie wykorzystano biblioteki \emph{OpenCV}, które wykorzystują mechanizm \href{https://en.wikipedia.org/wiki/Java_Native_Interface}{\emph{Java-Native-Interface}}. Biblioteka \emph{OpenCV} została napisana w C++ w ten sposób, aby dało się ją skompilować dla różnych systemów operacyjnych, w tym m.in. dla \emph{Linux'a} i \emph{Windowsa}. W ramach projektu została skompilowana ta sama wersja biblioteki dla 64-bitowych systemów \emph{Windows} i \emph{Linux} (odpowiednio biblioteka: \texttt{opencv\_java310.dll} i \texttt{libopencv\_java310.so}) z drobną modyfikacją w kodzie C++ (patrz:~\ref{sec:trudnosci}). W trakcie testów przetestowano z powodzeniem aplikację na obu tych systemach.
...
W przypadku chęci wygodnego konfigurowania, edytowania i \emph{debugowania} kodu źródłowego projektu, należy zainstalować dodatkowe pakiety oprogramowania, takie jak:
\begin{enumerate}
	\item \texttt{git},
	\item \texttt{eclipse},
	\item \texttt{mc},
...
\end{lstlisting}
\begin{sloppypar}
Wypełnienie tabeli zostało wykonane automatycznie w \emph{Javie}, po wywołaniu funkcji \texttt{addAllFacesFromPredefinedCsv()} z pliku \texttt{DatabaseConnectionManager.java}. Funkcja ta czyta plik CSV, który został wygenerowany prostym skryptem, napisanym w \emph{Pythonie}, na podstawie folderu \texttt{YaleFacedatabaseA}, który zawiera zdjęcia twarzy. Zdjęcia te można pobrać ze strony internetowej \emph{Computer Vision Laboratory in the Computer Science and Engineering Department} Uniwersytetu Kalifornijskiego w San~Diego:
\begin{center}
\url{http://vision.ucsd.edu/datasets/yale_face_dataset_original/yalefaces.zip}
\end{center}
Notka licencyjna z pliku \texttt{Readme.txt}, rozpakowanego z ww. archiwum \texttt{yalefaces.zip}:
\begin{displayquote}
...
Aplikację zaimplementowano w języku \emph{Java} w wersji \emph{8}, korzystając ze środowiska graficznego \emph{Swing}.
...
Aby uruchomić aplikację w trybie \texttt{debug}, należy włączyć \emph{VirtualBox}, uruchomić maszynę wirtualną z \emph{Debianem}, zalogować się, wybrać z menu: \texttt{Programy > Programowanie > Eclipse > Run > Debug}. W przypadku gdyby \emph{Eclipse} pytał, który plik źródłowy uruchomić, należy wskazać \texttt{MainWindow.java}.
...
Inną trudnością związaną również z \emph{wrapperem} od \emph{Javy}, jest to, że nie wszystkie funkcje dostępne w interfejsie C++ \emph{OpenCV} są dostępne w \emph{wraperze} w \emph{Javie}. Jedną z istotnych funkcji niedostępnych w \emph{Javie} jest przeciążona\footnote{\emph{Przeciążona funkcja} w sensie jednej nazwy funkcji z różnymi zestawami argumentów.} funkcja \texttt{predict}, która w \emph{Javie} jest dostępna tylko w jednej wersji, w której zwracany jest jeden identyfikator osoby rozpoznanej na zadanym zdjęciu twarzy wraz z miarą odległości zdjęcia rozpoznawanego i tego, który wygenerował algorytm \emph{eigenfaces}. Problem ten został rozwiązany dzięki uprzejmości jednego z \emph{developerów} modułu \emph{OpenCV~Faces}, tj. dzięki \href{https://github.com/mshabunin}{\emph{Maksimowi Shabuninowi}}, który po mailowym kontakcie opisał rozwiązanie w komentarzu do pytania zamieszczonego na forum \href{http://answers.opencv.org}{\texttt{answers.opencv.org}}:
\begin{center}
...
\begin{tabular}{p{2.5cm}|p{4cm}|p{4cm}|p{4cm}}
& Patryk Bęza & Marek Kozak & Krzysztof Małaśnicki \\\hline\hline
\multirow{3}{*}{\parbox{3cm}{\ \\Opracowane \\funkcjonalności}} & $\bullet$ Obsługa algorytmu \emph{eigenfaces} w \emph{Javie} & -- & $\bullet$ Dodanie pierwszej wersji \emph{progress bar'a} i przeniesienie funkcjonalności z karty \emph{All faces} do \emph{Found faces}\\
